...
function promedio(cajaRubro:TM;n,m:byte):integer;
var
  suma :integer;
begin
  if n<1 then
...
procedure maxGan(cajaRubro:TM; productos:TVR;i,m;var aux:byte; var val:real);
begin
  if M=1 then
    begin
    aux:=1
    val:= cajaRubro[i,j]*(productos[m].margen)/100;
    end;
  else
...
begin
  if M<1 then
    ...
    if controlXCaja(cajaRubro,n,m,x) then
      ...
      incr:=0;
    cuentraRubros:= incr + cuentaRubros(cajaRubro,n,m-1,x);
    end;
end;
